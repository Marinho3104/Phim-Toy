...
bool parser::Compiler_Declarations::isDeclared(int __declaration_id) {
    ...
    return (getVariableDeclaration(__declaration_id) || getFunctionDeclaration(__declaration_id) || getStructDeclaration(__declaration_id));
...
parser::Ast_Node_Variable_Declaration* parser::Compiler_Declarations::getVariableDeclaration(int __declaration_id) {
    ...
    for (int _ = 0; _ < variable_declarations->count; _++)
        ...
        if ((*variable_declarations)[_]->declaration_id == __declaration_id) 

            return (*variable_declarations)[_];
...
parser::Ast_Node_Function_Declaration* parser::Compiler_Declarations::getFunctionDeclaration(int __declaration_id) {
    ...
    for (int _ = 0; _ < function_declarations->count; _++)
        ...
        if ((*function_declarations)[_]->declaration_id == __declaration_id) 

            return (*function_declarations)[_];
...
parser::Ast_Node_Struct_Declaration* parser::Compiler_Declarations::getStructDeclaration(int __declaration_id) {
    ...
    for (int _ = 0; _ < struct_declarations->count; _++)
        ...
        if ((*struct_declarations)[_]->declaration_id == __declaration_id) 

            return (*struct_declarations)[_];
...
parser::Compiler_Code_Block::Compiler_Code_Block(Name_Space* __name_space, Compiler_Code_Block* __previous) 
    : name_space(__name_space), previous(__previous) {
...
void parser::Compiler_Code_Block::generate(Compiler_Control* __compiler_control, parser::Ast_Node_Name_Space* __name_space_node, Compiler_Code_Block* __previous) {
    ...
    if (!_compiler_code_block) {
        ...
        __compiler_control->compiler_code_blocks->add(_compiler_code_block);
    } 
...
int parser::Compiler_Code_Block::generate(Compiler_Control* __compiler_control, parser::Ast_Node_Function_Declaration* __function_declaration, Compiler_Code_Block* __previous, int __position) {
    ...
    if (__position != -1) _compiler_code_block = (*__compiler_control->compiler_code_blocks)[__position];

    else {
        _compiler_code_block = (parser::Compiler_Code_Block*) malloc(sizeof(parser::Compiler_Code_Block));
        new (_compiler_code_block) parser::Compiler_Code_Block(__function_declaration->name_space, __previous);
        __position = __compiler_control->compiler_code_blocks->add(_compiler_code_block);
    }
...
parser::Compiler_Exception::Compiler_Exception(const char* __description) 
    : description(__description) 
        { std::cout << "Compiler Exception: " << description << std::endl; exit(1); }


parser::Compiler_Control::~Compiler_Control() { if (!built_in_code_blocks) delete implicit_values; else delete compiler_code_blocks; }
        
parser::Compiler_Control::Compiler_Control(
    utils::LinkedList <Ast_Node_Name_Space*>* __name_space_nodes, utils::LinkedList <char*>* __implicit_values, utils::LinkedList <Compiler_Code_Block*>* __built_in_code_blocks, bool __debug_info) 
    : name_space_nodes(__name_space_nodes), implicit_values(__implicit_values), built_in_code_blocks(__built_in_code_blocks), debug_info(__debug_info) {
        compiler_code_blocks = new utils::LinkedList <Compiler_Code_Block*>();
    }

parser::Compiler_Control::Compiler_Control(utils::LinkedList <Ast_Node_Name_Space*>* __name_space_nodes, utils::LinkedList <char*>* __implicit_values) 
    : name_space_nodes(__name_space_nodes), implicit_values(__implicit_values), built_in_code_blocks(NULL), debug_info(0) {
        compiler_code_blocks = new utils::LinkedList <Compiler_Code_Block*>();
    }

void parser::Compiler_Control::printDebugInfo(const char* __information) {
    std::cout << "Compiler Control - Debug Info: " << __information << std::endl << std::endl;
}
...
parser::Compiler_Code_Block* parser::Compiler_Control::getCompilerCodeBlockFromNameSpace(Name_Space* __name_space) {
    ...
    for (int _ = 0; _ < compiler_code_blocks->count; _++)

        if ((*compiler_code_blocks)[_]->name_space == __name_space) return (*compiler_code_blocks)[_];
...
parser::Compiled_Output* parser::Compiler_Control::generateOutPut() {
    ...
    for (int _ = 0; _ < compiler_code_blocks->count; _++) {
        ...
        new (_block) Compiled_Code_Block(
            (*compiler_code_blocks)[_]->byte_code
        );
