...
parser::Ast_Node_Variable* parser::Ast_Node_Variable::generate(Ast_Control* _astCntrl) {
    ...
    int _declId = _astCntrl->crrntBlock->getNameDeclPos((*_astCntrl->tokensColl)[(_astCntrl->crrntTk)++]->phr);

    if (_declId == -1) exit(-1); // Error not defined variable TODO
...
parser::Ast_Node_Pointer_Operators::Ast_Node_Pointer_Operators(utils::LinkedList <int>* _operations, Ast_Node* _value) 
    : Ast_Node(AST_NODE_POINTER_OPERATORS), operations(_operations), value(_value) {}

parser::Ast_Node_Pointer_Operators* parser::Ast_Node_Pointer_Operators::generate(Ast_Control* _astCntrl) {
    std::cout << "--> Ast Node Pointer Operators <--" << std::endl;
    
    utils::LinkedList <int>* _operatorsUsed = new utils::LinkedList <int>();
    parser::Token* _tk;
    while((*_astCntrl->tokensColl)[(_astCntrl->crrntTk)]->id == TOKEN_ADDRESS || (*_astCntrl->tokensColl)[(_astCntrl->crrntTk)]->id == TOKEN_POINTER) 
        _operatorsUsed->add((*_astCntrl->tokensColl)[(_astCntrl->crrntTk)++]->id);
    
    _tk = (*_astCntrl->tokensColl)[(_astCntrl->crrntTk)];
    
    if (
        !parser::isIdentifierType(_tk) && _tk->id != TOKEN_OPENPARENTHESES
    ) { exit(-1); /* Error to handle TODO */ }
